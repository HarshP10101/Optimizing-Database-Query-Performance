...
AND order_id IN (SELECT order_id FROM Order_items GROUP BY order_id 
                HAVING COUNT(*) > (SELECT AVG(size) FROM (SELECT COUNT(*) as size FROM Order_items GROUP BY order_id)))
'''
