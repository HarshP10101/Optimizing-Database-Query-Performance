...
for num, database_name in enumerate(['E:/GITHUB_PRJ/database_benchmarking/Database-Performance-Benchmarking-Project/small.db', 'E:/GITHUB_PRJ/database_benchmarking/Database-Performance-Benchmarking-Project/medium.db', 'E:/GITHUB_PRJ/database_benchmarking/Database-Performance-Benchmarking-Project/large.db']):
    for scenario in results:
        ...
        if scenario == "User":
            ...
            connection.commit()
        random_values = cursor.execute("""SELECT customer_postal_code FROM Customers
                                       WHERE customer_id IN (SELECT customer_id FROM Orders 
                                       WHERE order_id IN (SELECT order_id FROM Order_items 
                                       GROUP BY order_id HAVING COUNT(*) > 1))""").fetchall()
        for i in range(50):
        ...
        if scenario == "Uninformed":
            ...
            cursor.execute("ALTER TABLE old_Sellers RENAME TO Sellers")
...
ax.set_title("Query 1 (Runtime is ms)")
ax.legend(loc="upper right")

plt.savefig("Q4_chart.png")
#################################################################################
